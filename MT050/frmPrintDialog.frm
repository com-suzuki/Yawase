...
Private Sub cmdExecute_Click()
    ...
    With objArPrint
        .Name = "商品マスタ一覧表"
        .objReport = objRpt
        .Connection = frmMt050.m_clsAdoSQL.Connection
        .SQL = "SELECT * FROM vw_MT050 WHERE Icode BETWEEN '" & txtIcode(0).Text & "' AND '" & txtIcode(1).Text & "'"
        .Caption = "商品マスタ一覧表"
...
Private Function DoValidationChecks() As Boolean
    ...
    If Trim(txtIcode(0).Text) = "" Then
        strErrMsg = "商品コードを入力してください。"
        txtIcode(0).SetFocus
        GoTo ErrorTrap:
    End If
    If Trim(txtIcode(1).Text) = "" Then
        strErrMsg = "商品コードを入力してください。"
        txtIcode(1).SetFocus
        GoTo ErrorTrap:
    End If
...
Private Sub txtIcode_GotFocus(Index As Integer)

    txtIcode(Index).BackColor = FOCUS_STOP_COLOR
    
End Sub

Private Sub txtIcode_LostFocus(Index As Integer)

    txtIcode(Index).BackColor = FOCUS_NO_COLOR
