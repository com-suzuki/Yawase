...
Begin VB.Form frmYpmf120 
   ...
   Begin imText6Ctl.imText imtFocusFirst 
      ...
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         ...
         Strikethrough   =   0   'False
      EndProperty
      DropDown        =   "frmYpmf120.frx":103B
      Key             =   "frmYpmf120.frx":1059
   ...
   Begin imText6Ctl.imText imtFocusEnd 
      ...
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         ...
         Strikethrough   =   0   'False
      EndProperty
      DropDown        =   "frmYpmf120.frx":110B
      Key             =   "frmYpmf120.frx":1129
      BackColor       =   -2147483643
      EditMode        =   0
      ForeColor       =   -2147483640
      ReadOnly        =   0
   ...
   Begin imText6Ctl.imText imtIcode_Kana_Focus1 
      ...
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         ...
         Strikethrough   =   0   'False
      EndProperty
      DropDown        =   "frmYpmf120.frx":11DB
      Key             =   "frmYpmf120.frx":11F9
      BackColor       =   -2147483643
      EditMode        =   0
      ForeColor       =   -2147483640
      ReadOnly        =   0
...
Private Function MakePrintWork() As Boolean
    ...
    'ワーク削除
    strSQL = "DELETE FROM WK_YPMF120"
    g_clsAdoAccess.Connection.Execute strSQL

    'ワークオープン
    strSQL = "SELECT * FROM WK_YPMF120"
    wkRecordset.Open strSQL, g_clsAdoAccess.Connection, adOpenKeyset, adLockOptimistic
    
    '買主精算データオープン
    strSQL = "{call sp_YPMF1201;1('" & Trim(lblOdate.Caption) & "'," & cboBcode(0).Text & "," & cboBcode(1).Text & ")}"
    ...
    Do While Not adoDT041.EOF
        wkRecordset.AddNew
        If Trim(adoDT041.Fields("Odate")) = Trim(lblOdate.Caption) Then
            wkRecordset.Fields("Div") = 0
        Else
            wkRecordset.Fields("Div") = 1
        End If
        wkRecordset.Fields("Bcode") = adoDT041.Fields("Bcode")
        wkRecordset.Fields("Bname") = Global_Get_Bname(g_clsAdoSQL, adoDT041.Fields("Bcode"), adoDT041.Fields("Odate"), strBuff)
        wkRecordset.Fields("Odate") = adoDT041.Fields("Odate")
        wkRecordset.Fields("Price") = adoDT041.Fields("Total")
        wkRecordset.Fields("Tax") = adoDT041.Fields("Tax")
        wkRecordset.Fields("Total") = CCur(wkRecordset.Fields("Price")) + CCur(wkRecordset.Fields("Tax"))
        wkRecordset.Fields("Keep") = adoDT041.Fields("Keep")
        wkRecordset.Fields("GTotal") = adoDT041.Fields("GTotal")
        
        wkRecordset.Fields("Nyukin") = 0
        
        '入金データオープン
        strSQL = "{call sp_YPMF1202;1('" & wkRecordset.Fields("Odate") & "'," & wkRecordset.Fields("Bcode") & ")}"
        adoDT060.Open strSQL, g_clsAdoSQL.Connection, adOpenKeyset, adLockReadOnly
        If adoDT060.EOF = False Then
            If Not IsNull(adoDT060.Fields("Ptotal")) Then
                wkRecordset.Fields("Nyukin") = adoDT060.Fields("Ptotal")
            End If
        End If
        adoDT060.Close
        ...
        '残金がない場合はデータ作成しない
        If (CCur(wkRecordset.Fields("GTotal")) - CCur(wkRecordset.Fields("Nyukin"))) <= 0 Then
            wkRecordset.CancelUpdate
        Else
            wkRecordset.Update
        End If
...
'    '買主精算データオープン(累積)
'    strSQL = "{call sp_YPMF1203;1('" & Trim(lblOdate.Caption) & "'," & cboBcode(0).Text & "," & cboBcode(1).Text & ")}"
'    adoDT041.Open strSQL, g_clsAdoSQL.Connection, adOpenKeyset, adLockReadOnly
...
'
'        '入金データオープン(累積)
'        strSQL = "{call sp_YPMF1204;1('" & wkRecordset.Fields("Odate") & "'," & wkRecordset.Fields("Bcode") & ")}"
'        adoDT060.Open strSQL, g_clsAdoSQL.Connection, adOpenKeyset, adLockReadOnly
'        If adoDT060.EOF = False Then
'            If Not IsNull(adoDT060.Fields("Ptotal")) Then
'                wkRecordset.Fields("Nyukin") = adoDT060.Fields("Ptotal")
