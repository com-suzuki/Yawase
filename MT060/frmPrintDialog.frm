...
Private Sub cmdExecute_Click()
    ...
    With objArPrint
        .Name = "商品区分マスタ一覧表"
        .objReport = objRpt
        .Connection = frmMt060.m_clsAdoSQL.Connection
        .SQL = "SELECT * FROM vw_MT060 WHERE Dcode BETWEEN '" & txtDcode(0).Text & "' AND '" & txtDcode(1).Text & "'"
        .Caption = "商品区分マスタ一覧表"
...
Private Function DoValidationChecks() As Boolean
    ...
    If Trim(txtDcode(0).Text) = "" Then
        strErrMsg = "商品区分を入力してください。"
        txtDcode(0).SetFocus
        GoTo ErrorTrap:
    End If
    If Trim(txtDcode(1).Text) = "" Then
        strErrMsg = "商品区分を入力してください。"
        txtDcode(1).SetFocus
        GoTo ErrorTrap:
    End If
...
Private Sub txtDcode_GotFocus(Index As Integer)

    txtDcode(Index).BackColor = FOCUS_STOP_COLOR
    
End Sub

Private Sub txtDcode_LostFocus(Index As Integer)

    txtDcode(Index).BackColor = FOCUS_NO_COLOR
